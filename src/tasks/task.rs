...
impl Display for Task {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        if self.priority.is_some() && !self.completed {
            f.write_fmt(format_args!("({}) ", self.priority.unwrap() as char))?;
        }
...
impl FromStr for Task {
    ...
    fn from_str(mut s: &str) -> Result<Self, Self::Err> {
        ...
        // string end with a tag.
         let subject = String::from_utf8(subject).unwrap_or_else(|_| s.to_owned()).trim_end().to_string();
...
#[cfg(test)]
mod tests {
    use super::{NaiveDate, Task, HashMap, Local};
